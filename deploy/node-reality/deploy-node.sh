#!/usr/bin/env bash
# RFPlay — Node deploy script (REALITY variant)
#
# Installs Xray-core + the RFPlay daemon on a fresh Ubuntu/Debian VPS and
# wires them together with systemd. The daemon pulls the node's Xray config
# from the Manager, applies it, restarts Xray on change, and reports per-user
# traffic.
#
# Usage:
#   sudo ./deploy-node.sh --manager-url https://airport.example.com \
#                         --node-token nd_xxxxxxxxxxxxxxxxxxxxxxxx
#
# Requirements:
#   - root (or sudo)
#   - Manager API reachable from the VPS
#   - A node created in the Admin panel with a rotated token (nodes/:id/token)
set -euo pipefail
...
if [[ ! -x "$DAEMON_BIN" ]]; then
  log "building and installing rfplay-daemon (requires Go toolchain)"
...
# --- 3. Daemon config ---
# The daemon writes the fetched Xray config to DATA_DIR/xray.json and restarts
# Xray with `-c` pointing there. node_id is informational; the token is the
# real identity on the Manager side.
...
# --- 5. systemd unit for Xray ---
# The daemon manages the process lifecycle itself (restarts on config change),
# so this unit is a plain `xray -c <daemon-managed config>` that stays up.
...
log "daemon + xray started."
log "  journalctl -u rfplay-daemon -f"
log "  journalctl -u rfplay-xray -f"
log "  xray config will be written to /var/lib/rfplay/xray.json on first sync."
