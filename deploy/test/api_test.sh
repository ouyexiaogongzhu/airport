...
HTTP_CODE=*** -so /dev/null -w '%{http_code}' -X POST $BASE/public/register \
  -H 'Content-Type: application/json' \
  -d '{"username":"ittest","password":"test123456"}')
...
# 5. Login
echo "--- 5. Login ---"
LOGIN_RESP=*** -s -X POST $BASE/public/login \
  -H 'Content-Type: application/json' \
  -d '{"username":"ittest","password":"test123456"}')
...
HTTP_CODE=*** -so /dev/null -w '%{http_code}' -X POST $BASE/public/login \
  -H 'Content-Type: application/json' \
...
echo "--- 7. Admin Login ---"
ADMIN_LOGIN=*** -s -X POST $BASE/public/login \
  -H 'Content-Type: application/json' \
...
if [ -n "$ADMIN_TOKEN" ]; then
    ...
    ADMIN_REG=*** -s -X POST $BASE/public/register \
      -H 'Content-Type: application/json' \
      -d '{"username":"admin","password":"admin123"}')
...
if [ -n "$ADMIN_TOKEN" ] && [ "$ADMIN_TOKEN" != "$TOKEN" ]; then
    NODE_RESP=*** -s -X POST $BASE/admin/nodes \
      -H 'Content-Type: application/json' \
...
# 9. List Nodes
echo "--- 9. List Nodes ---"
HTTP_CODE=*** -so /dev/null -w '%{http_code}' $BASE/admin/nodes \
...
ORDER_RESP=*** -s -X POST $BASE/user/orders \
  -H 'Content-Type: application/json' \
...
PAY_RESP=*** -s -X POST $BASE/public/payment/callback \
  -H 'Content-Type: application/json' \
...
ORDERS_RESP=*** -s $BASE/user/orders \
  -H "Authorization: Bearer $TOKEN")
...
HTTP_CODE=*** -so /dev/null -w '%{http_code}' -X POST $BASE/public/payment/callback \
  -H 'Content-Type: application/json' \
  -d "{\"order_id\":$ORDER_ID,\"status\":\"paid\"}")
...
if [ -n "$ADMIN_TOKEN" ] && [ "$ADMIN_TOKEN" != "$TOKEN" ]; then
    HTTP_CODE=*** -so /dev/null -w '%{http_code}' -X POST $BASE/admin/traffic/report \
      -H 'Content-Type: application/json' \
      -H "Authorization: Bearer $ADMIN_TOKEN" \
...
if [ -n "$ADMIN_TOKEN" ] && [ "$ADMIN_TOKEN" != "$TOKEN" ]; then
    HTTP_CODE=*** -so /dev/null -w '%{http_code}' "$BASE/admin/traffic/stats" \
      -H "Authorization: Bearer $ADMIN_TOKEN")
...
HTTP_CODE=*** -so /dev/null -w '%{http_code}' $BASE/user/profile \
  -H "Authorization: Bearer $TOKEN")
