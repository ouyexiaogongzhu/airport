...
| 38 | Hysteria2 / gRPC | 不支持；XHTTP 见 [docs/xhttp-cloudflare-design.md](docs/xhttp-cloudflare-design.md) |
...
生产节点均为 `xhttp`（path `/rfhttp/`）。`security` / `reality_*` 列停用（暂不删列）。
...
- #38：Hysteria2 等；XHTTP 已实现并文档化（[docs/xhttp-cloudflare-design.md](docs/xhttp-cloudflare-design.md)）
